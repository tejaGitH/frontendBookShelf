...
import { markBookAsCurrentlyReading, updateReadingProgress, markBookAsFinished, fetchReadingProgressForCurrentBooks, fetchCurrentBooks } from '../../../actions/bookActions';
import { fetchBooks, fetchBestSellers,fetchFinishedBooks } from '../../../actions/bookActions';
...
const MyBooks = () => {
  ...
  useEffect(() => {
    ...
    if (storedProgress !== null && storedProgress !== undefined && storedProgress !== 'undefined') {
  ...
  const handleMarkAsFinished = (bookId) => {
    ...
    dispatch(fetchFinishedBooks()); // Ensure finished books are fetched immediately
    handleClearSelection();
  };
      ...
      <div className="book-details">
        ...
        {selectedBook.finished ? (
          <div>
            ...
            <p><strong>Review:</strong> {selectedBook.review}</p>
          </div>
        ) : (
          <div>
            ...
            <button onClick={() => handleUpdateProgress(selectedBook._id, comments)}>Update Progress</button>
            <button onClick={() => handleMarkAsFinished(selectedBook._id)}>Mark as Finished</button>
          </div>
        )}
      </div>
    </div>
